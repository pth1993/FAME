...
data_loader_test = DataLoader(data_test, batch_size=32, shuffle=False, collate_fn=CustomCollateGE())

data_test_ext = L1000XPRDataset(data_file='data/lincs/signature_xpr_cl.csv')
data_loader_test_ext = DataLoader(data_test_ext, batch_size=2, shuffle=False)

fcd = FCD(device=device, n_jobs=8)

data_excapedb = read_pickle('data/lincs/excapedb_calculated.pkl')

model = CFAME(num_node_features=len(atom_dict['c2i']), num_edge_features=len(bond_dict['c2i']), n_gnn_layers=5,
...
with torch.no_grad():
    ...
    for i, b in enumerate(tqdm(data_loader_test_ext)):
        ...
        target = b['target']
        if isinstance(model, nn.DataParallel):
            out = model.module.sample(ge, neighbor=neighbor, num_latent=50, num_sample_per_latent=50, max_length=12,
                                      topk=50, atom_dict=atom_dict, bond_dict=bond_dict, fragment_dict=fragment_dict)
        else:
            out = model.sample(ge, neighbor=neighbor, num_latent=50, num_sample_per_latent=50, max_length=12, topk=100,
                               atom_dict=atom_dict, bond_dict=bond_dict, fragment_dict=fragment_dict)
        output_ext += out
